...
def create_app(test_config=None):
    ...
    checkin_score_scheduler.add_job(checkin_score, trigger='interval', seconds=30)
    ...
    overall_scheduler.add_job(overall_reviewes, trigger='interval', seconds=30)
    ...
    reset_scheduler.add_job(update_croncheckin, trigger='interval', seconds=160)
    reset_scheduler.start()

    recent_activity_scheduler = BackgroundScheduler()
    recent_activity_scheduler.add_job(recent_activity, trigger='interval', seconds=30)
    recent_activity_scheduler.start()

    reviewed_activity_scheduler = BackgroundScheduler()
    reviewed_activity_scheduler.add_job(reviewed_activity, trigger='interval', seconds=30)
