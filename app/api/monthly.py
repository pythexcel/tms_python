    ...
    if request.method == "GET":
        ...
        if len(rep) >= 3:
            ...
            if rep is not None:
                ...
                notification_message = requests.post(url=notification_system_url+"notify/dispatch?account-name=notify_tms",json=monthly_payload)
    ...
    for data in dab:
        ...
        for dub in rap:
            ...
            if not sap:
                ...
                notification_message = requests.post(url=notification_system_url+"notify/dispatch?account-name=notify_tms",json=monthly_reviewed_payload)
                return jsonify(str(ret)), 200
            else:
